...
library SolRsaVerify {
        ...
        returns (uint256)
    {
        unchecked {
            ...
            require(_m.length >= sha256Prefix.length + _sha256.length + 11, "pkcs1Sha256Verify: _m.length is incorrect");
    ...
    function pkcs1Sha256VerifyStr(string memory _data, bytes memory _s, bytes memory _e, bytes memory _m)
        internal
        view
        returns (uint256)
    {
